    ...
    <div class="modal-content">
      ...
      <div class="modal-body form">
        <form action="#" id="form" class="form-horizontal">
          <input type="hidden" value="" name="kd_tindakan"/> 
          <div class="form-body">

            <div class="form-group">
              <label class="control-label col-md-2">Nama Tindakan</label>
              <div class="col-md-9">
                <input name="nm_tindakan" id="nm_tindakan" placeholder="Nama Tindakan" class="form-control" type="text">
             </div>
           </div>
...
<script src="<?=base_url()?>assets/admin/bower_components/jquery/dist/jquery.min.js"></script>
    ...
    $(document).ready(function() { 
      ...
      table = $('#table').DataTable({ 
        ...
        "ajax": {
          "url": "<?php echo base_url() ?>master/Tindakan/list_data",
          "type": "POST"
    ...
    function save()
    {
      ...
      if(save_method == 'add') 
      {
        url = "<?php echo site_url('master/Tindakan/ajax_add')?>";
        pesan = "Tambah Data";
      }
      else
      {
        url = "<?php echo site_url('master/Tindakan/ajax_update')?>";
        pesan = "Edit Data";
      }
     ...
     function edit_data(id)
     {
      ...
      $.ajax({
        url : "<?php echo site_url('master/Tindakan/ajax_get_data_by_id')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

          $('[name="kd_tindakan"]').val(data.kd_tindakan);
          $('[name="nm_tindakan"]').val(data.nm_tindakan);
    ...
    function delete_data(id){
      ...
      function(isConfirm){
        if (isConfirm){
          $.ajax({
            url : "<?php echo site_url('master/Tindakan/delete_via_ajax')?>",
            type: "POST",
            data:{"id":id},
            dataType: "JSON",
          success: function(data)
